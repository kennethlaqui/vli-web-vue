...
export default {
  ...
  watch: {
    reloadSection: function (value) {
      this.loadSection()
      this.$root.$emit('reloadSection', false)
    }
  },
  methods: {
    buildBuildItem () {
    ...
    loadSection () {
      this.$store.dispatch('retrieveSectionData', {
        primekey: this.primekey
      })
        .then(() => {
          this.section = this.$store.getters.retrieveSectionData
        })
    }
  },
  created () {
